...
export function LineChart({ title, subtitle, points, color, markers = [], footer }: LineChartProps) {
  return (
      ...
      <svg viewBox="0 0 600 220" role="img" aria-label={title}>
        ...
        {markers.map((marker) => (
          <g key={`${marker.x}-${marker.y}-${marker.label ?? ''}`}>
            <line x1={marker.x} x2={marker.x} y1="24" y2="200" className="payment-marker-line" />
            <circle cx={marker.x} cy={marker.y} r="6" className="payment-marker-dot" />
          </g>
        ))}
      </svg>
